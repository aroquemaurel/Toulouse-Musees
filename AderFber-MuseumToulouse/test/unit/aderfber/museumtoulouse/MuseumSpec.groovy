...
class MuseumSpec extends Specification {
    void "test a valid museum"(pname, popeninghours, pphone, psubwayAccess, pbusAccess) {
        ...
        Museum museum = new Museum(name: pname, openingHours: popeninghours, phone: pphone,
                ...
                busAccess: pbusAccess, address: Mock(Address))
    ...
    void "test an unvalid museum"(pname, popeninghours, pphone, psubwayAccess, pbusAccess, paddress) {
        given: "an initialize museum"
        Museum museum = new Museum(name: pname, openingHours: popeninghours, phone: pphone,
                subwayAccess: psubwayAccess,
                busAccess: pbusAccess, address: paddress)

        expect: "Museum is valid"
        !museum.validate()

        where:
        pname                                                       |       popeninghours                                                   |       pphone          | psubwayAccess     |       pbusAccess  | paddress
        ""                                                          |   "Ouvert tous les jours de 9h à 19h."                                |   "05 61 36 81 12"    |   "Roseraie (A)"  |   "36, 38"        |   Mock(Address)
        null                                                        |   "Tous les jours : 10h - 18h /  nocturne le mercredi jusqu'à 21h."   |   ""                  |   "Esquirol (A)"  |   "2, 10, 24"     |   Mock(Address)
        "MUSEE PAUL-DUPUY - ARTS GRAPHIQUES ET ARTS DECORATIFS"     |   ""                                                                  |   null                |   "Capitole (A)"  |   "2"             |   Mock(Address)
        "MUSEE DE L'HISTOIRE DE LA MEDECINE DE TOULOUSE"            |   null                                                                |   "05 61 77 82 72"    |   ""              |   "42"            |   Mock(Address)
        "MUSEE DE L'HISTOIRE DE LA MEDECINE DE TOULOUSE"            |   "Ouvert du mardi au samedi de 13h à 19hfermé les dimanches"         |   "05 61 77 82 72"    |   null            |   "10"            |   null
    }
}
